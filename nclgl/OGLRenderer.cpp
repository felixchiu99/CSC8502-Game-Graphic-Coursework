...
void OGLRenderer::UpdateShaderMatrices()	{
	if(currentShader) {
		/* 
		//UBO  (can pass a multiple type buffer by passing a struct)
		shaderMatrices[0] = modelMatrix.values;
		shaderMatrices[1] = viewMatrix.values;
		shaderMatrices[2] = projMatrix.values;
		shaderMatrices[3] = textureMatrix.values;
		shaderMatrices[4] = shadowMatrix.values;


		int binding = 5;		//where the
		GLuint someBuffer = 0;
		glGenBuffers(1, &someBuffer);
		glBindBuffer(GL_UNIFORM_BUFFER, someBuffer);
		glBufferData(GL_UNIFORM_BUFFER, sizeof(Matrix4)*5, shaderMatrices, GL_DYNAMIC_DRAW);
		glBindBufferBase(GL_UNIFORM_BUFFER, binding, someBuffer);

		int ubo = glGetUniformBlockIndex(currentShader->GetProgram(), "ObjectMatrices");	// get the magic number of the "object" you want to bind the buffer to
		glUniformBlockBinding(currentShader->GetProgram(), ubo, binding);					// tell the shader to look at slot <binding> for the data.
		//*/
...
void OGLRenderer::SetShaderLight(const Light& l) {
	...
	glUniform3fv(glGetUniformLocation(currentShader->GetProgram(),
		"lightDirection"), 1, (float*)& l.GetDirection());
